...
export function AlertCenterPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9] p-3 sm:p-6 font-sans text-slate-800">
      <div className="max-w-6xl mx-auto space-y-4 sm:space-y-5">

        {/* 1. PREMIUM HEADER */}
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <nav className="flex items-center gap-1.5 text-[11px] uppercase tracking-widest text-slate-400 font-bold mb-2 sm:mb-3">
            ...
            <div className="flex items-center gap-2.5 sm:gap-3">
              <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-slate-900">Event Console</h1>
            ...
            <p className="text-xs sm:text-sm text-slate-500 font-medium mt-1">Real-time anomaly detection and response tracking.</p>
          </div>

          <div className="relative z-10 flex gap-2 bg-slate-50 border border-slate-100 rounded-lg p-1.5 sm:p-2 w-full md:w-auto justify-around sm:justify-start">
            <div className="px-3 sm:px-4 py-1.5 sm:py-2 text-center">
              ...
              <p className={`font-mono text-lg sm:text-xl font-bold ${critCount > 0 ? 'text-rose-600' : 'text-slate-700'}`}>{critCount}</p>
            </div>
            ...
            <div className="px-3 sm:px-4 py-1.5 sm:py-2 text-center">
              ...
              <p className={`font-mono text-lg sm:text-xl font-bold ${warnCount > 0 ? 'text-amber-600' : 'text-slate-700'}`}>{warnCount}</p>
            </div>
            <div className="w-px bg-slate-200 mx-0.5 sm:mx-1"></div>
            <div className="px-3 sm:px-4 py-1.5 sm:py-2 text-center">
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Un-Ack</p>
              <p className="font-mono text-lg sm:text-xl font-bold text-slate-700">{unackCount}</p>
            </div>
          </div>
        </div>
        ...
        <div className="w-full overflow-x-auto pb-1 scrollbar-none touch-pan-x">
          <div className="flex gap-1.5 bg-white rounded-xl border border-slate-200/80 shadow-sm p-1.5 w-fit min-w-max">
            {['All', 'Critical', 'Warning', 'Resolved'].map((tab) => (
              <button
                key={tab}
                onClick={() => setFilter(tab)}
                className={`px-3.5 sm:px-5 py-1.5 sm:py-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-widest rounded-lg transition-all ${
                  filter === tab
        ...
        <div className="space-y-4">
          {filtered.length === 0 && (
            <div className="bg-white rounded-xl border border-slate-200/80 p-8 sm:p-12 text-center shadow-sm">
              <ShieldCheck className="w-10 h-10 sm:w-12 sm:h-12 text-emerald-400 mx-auto mb-3 opacity-80" />
          ...
          {filtered.map((alert) => {
            ...
            return (
              ...
              >
                ...
                <div className={`w-full h-1 md:w-2 md:h-auto shrink-0 ${
                ...
                <div className="p-3.5 sm:p-5 flex-1 flex flex-col">
                  ...
                  <div className="flex items-start gap-3 sm:gap-4 mb-3 sm:mb-4">
                    ...
                    <div className="min-w-0">
                      <h3 className="text-sm sm:text-base font-bold text-slate-900 leading-snug">{alert.title}</h3>
                      <p className="text-[10px] sm:text-[11px] font-bold text-slate-500 uppercase tracking-wider mt-0.5 mb-1">{alert.sysName}</p>
                      <p className="text-xs sm:text-sm text-slate-600 font-medium leading-relaxed">{alert.message}</p>
                    </div>
                  </div>

                  {/* Standard Operating Procedure (SOP) Box */}
                  {!isResolved && (
                    <div className="mt-2 sm:mt-auto bg-slate-50/50 border border-slate-100 rounded-lg p-2.5 sm:p-3 sm:ml-13">
                ...
                {/* Right Action Panel */}
                <div className="bg-slate-50 border-t md:border-t-0 md:border-l border-slate-100 p-3.5 sm:p-5 flex flex-col justify-center gap-2.5 sm:gap-3 w-full md:w-56 shrink-0">
                  {!isResolved && !isAcknowledged && (
                    <button
                      onClick={() => acknowledge(alert.id)}
                      className="w-full py-2 bg-white border border-slate-200 shadow-sm text-slate-700 text-[11px] font-bold uppercase tracking-wider rounded-lg hover:bg-slate-100 hover:border-slate-300 transition-colors flex items-center justify-center gap-2 cursor-pointer"
                    >
                  ...
                  {!isResolved && (
                    <button
                      ...
                      className="w-full py-2 bg-emerald-50 border border-emerald-200 shadow-sm text-emerald-700 text-[11px] font-bold uppercase tracking-wider rounded-lg hover:bg-emerald-100 transition-colors flex items-center justify-center gap-2 cursor-pointer"
                    >
                  ...
                  {alert.systemId && (
                    <Link
                      ...
                      className="mt-1 sm:mt-2 w-full flex items-center justify-center gap-1.5 text-[11px] font-bold text-sky-600 hover:text-sky-700 uppercase tracking-widest bg-sky-50/50 py-1.5 rounded-lg transition-colors"
                    >
